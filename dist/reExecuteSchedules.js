...
exports.reExecuteSchedules = reExecuteSchedules;
const logger_1 = require("./logger");
const cache_1 = require("./cache");
const executePin_1 = require("./executePin");
async function reExecuteSchedules(controllers, dependencies = {}) {
    const { getCachedData: getCachedDataFn = cache_1.getCachedData, isCacheValid: isCacheValidFn = cache_1.isCacheValid, executeSchedule: executeScheduleFn = executePin_1.executeSchedule } = dependencies;
    const results = [];
    for (const controller of controllers) {
        ...
        try {
            ...
            if (!cachedData) {
                ...
                (0, logger_1.logWarn)(`[CACHE] No cached data found for location=${controller.locationUuid}, device=${controller.uuid}`);
            ...
            if (!isCacheValidFn(cachedData)) {
                ...
                (0, logger_1.logWarn)(`Cache expired for controller ${controller.identifier} (${cacheAge.toFixed(1)} minutes old)`);
                results.push(controllerResult);
                continue;
            }
            const pinsToUpdate = {};
            ...
            for (const [pin, pinData] of Object.entries(controller.pins)) {
                ...
                if (!schedule) {
                    ...
                    (0, logger_1.logWarn)(`[SCHEDULE] Missing schedule for pin ${pin} on controller ${controller.identifier}`);
                    controllerResult.pinsExecuted.push(pinResult);
                    continue;
                }
                const { shouldExecute, localTime } = (0, executePin_1.shouldExecutePin)(schedule, currentStatus, controller.timezone);
            ...
            if (updatedPins.length > 0) {
                try {
                    await executeScheduleFn(controller, pinsToUpdate);
                    (0, logger_1.logInfo)(`Executed schedule for pins [${updatedPins.join(', ')}] on ${controller.identifier}`);
                }
                catch (error) {
                    controllerResult.success = false;
                    controllerResult.error = `Failed to execute schedule updates: ${error instanceof Error ? error.message : String(error)}`;
                    (0, logger_1.logError)(`Error executing schedule for ${controller.identifier}: ${error}`);
                }
            }
            if (notUpdatedPins.length > 0) {
                (0, logger_1.logInfo)(`No need to reExecute pins [${notUpdatedPins.join(', ')}] on ${controller.identifier}`);
            }
            results.push(controllerResult);
        }
        catch (error) {
            ...
            results.push(controllerResult);
            (0, logger_1.logError)(`Error processing ${controller.identifier}: ${error}`);
        }
    }
